...
def showText(text):
    ...
    pressKeySurf, pressKeyRect = txtObjects('Press any key to continue', basic_font, title_color)
...
def drawTitle():
    titleSurf = big_font.render('Tetris Lite', True, title_color)
...
def drawInfo(points, level):
    pointsSurf = basic_font.render(f'Points: {points}', True, txt_color)
    ...
    pausebSurf = basic_font.render('Pause: space', True, info_color)
    pausebRect = pausebSurf.get_rect()
    pausebRect.topleft = (window_w - 550, 420)
    display_surf.blit(pausebSurf, pausebRect)

    escbSurf = basic_font.render('Exit: Esc', True, info_color)
